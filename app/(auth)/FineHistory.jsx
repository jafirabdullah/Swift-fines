...
const FineHistory = () => {
  ...
  const fetchFineHistory = async () => {
    try {
      ...
      const { data } = await axios.get('http://192.168.8.111:8080/api/v1/fines/history', {
        headers: {
      ...
      Alert.alert('Error', error.response?.data?.message || 'An error occurred while fetching fine history');
      setIsLoading(false);
    }
  };
